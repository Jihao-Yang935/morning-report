...
10. 资金面深度专节（必须使用上方实时金融数据中的北向资金净流入/融资余额变化/龙虎榜数据，结合机构调研动向，给出资金背书判断）
11. 👑龙头股跟踪专栏（3板以上全部龙头，每条包含：连板数+股票名称+代码+首封时间+封单金额+所属题材+涨停逻辑分析+明日预期（加速/分歧/断板风险），最高板和次高板重点分析）
12. L1-L4分层事件（按星级排序）
13. 昨日盘后公告快报
14. 今日风险清单
15. 事件日历
